...
export default function SimpleResourceDetail({ moduleDef, mode }) {
  ...
  const {
    ...
    getDeleteRedirectPath: () =>
      tenantSlug ? moduleDef.routes.list(tenantSlug) : moduleDef.routes.list(),
  ...
  return (
    <form className="flex flex-col gap-2 p-2" onSubmit={handleSubmit(onSubmit)}>
      <ControlBar>
        <ControlBar.Section align="start" priority={1}>
          {!inEditMode && showCreate && canCreate && (
            <Button type="submit" disabled={!isDirty} className="cursor-pointer">
              <Save /> {labels.create || "Create"}
            </Button>
          )}
          {inEditMode && (
            <>
              ...
              {showDelete && canDelete && (
                <Button type="button" onClick={remove} className="cursor-pointer">
                  <Trash /> {labels.delete || "Delete"}
                </Button>
              )}
            </>
          )}
      ...
      <DetailFieldsSection
        title={moduleDef.detail.sectionTitle}
        description={moduleDef.detail.sectionDescription}
